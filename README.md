...
|---|---|---|---|
| **agent-handbook** | 0.2.0 | A handbook you hand to your agent — teaching material for the general judgement calls: architecture decisions, planning, the rules that hold whatever is being built. Not tied to a language, a stack or a build system; nothing installs a runtime, opens a network connection or needs hardware. Its subjects include **`/target-world-walk`**, a portfolio-completeness instrument: derive what a plan needs by replaying one concrete unit of work through the system you *intend to have*, instead of auditing the tickets you already wrote — the forward companion to a backward carrier audit, which is blind by construction to anything the plan never named. It refuses a goal that points at a tracker item, classifies every need as covered / partial / missing / oral, and runs an ordering pass before anything is filed. | [README](plugins/agent-handbook/README.md) |

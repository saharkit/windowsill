...
| **target-world-walk** | 0.1.0 | A portfolio-completeness instrument, shipped as a single skill. Derive what a plan needs by replaying one concrete unit of work through the system you *intend to have*, instead of auditing the tickets you already wrote — the forward companion to a backward carrier audit, which is blind by construction to anything the plan never named. Refuses a goal that points at a tracker item, classifies every need as covered / partial / missing / oral, and runs an ordering pass before anything is filed. No install, no runtime, no hardware. | [README](plugins/target-world-walk/README.md) |

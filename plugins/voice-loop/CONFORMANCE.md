...
| 3.12 | Unheard line accounted for | Any turn whose 🔊 line was NOT heard — and, separately, ANY ordinary turn (one with no 🔊 line in it at all) | `speak.log` has a line saying why (give-up, dedup, the ledger, nothing marked, speech switched off, synthesis failure). EMPTY and IDENTICAL waits use the same vocabulary but remain distinguishable. A Stop exit with NO reason line is mechanically backstopped as `stop: exited with no reason recorded` — a turn with NO log line at all is a FAIL | | |

...
|---|---|---|---|
| **engineering-method** | 0.2.0 | Instruments for judging your own work — architecture decisions, planning, the general rules that hold whatever you are building. Not tied to a language, a stack or a build system; nothing installs a runtime, opens a network connection or needs hardware. Its instruments include **`/target-world-walk`**, a portfolio-completeness instrument: derive what a plan needs by replaying one concrete unit of work through the system you *intend to have*, instead of auditing the tickets you already wrote — the forward companion to a backward carrier audit, which is blind by construction to anything the plan never named. It refuses a goal that points at a tracker item, classifies every need as covered / partial / missing / oral, and runs an ordering pass before anything is filed. | [README](plugins/engineering-method/README.md) |

...
|---|---|---|---|
| **agent-handbook** | 0.3.0 | A handbook you hand to your agent — teaching material for the general judgement calls: architecture decisions, planning, the rules that hold whatever is being built. Not tied to a language, a stack or a build system; nothing installs a runtime, opens a network connection or needs hardware. It holds two kinds of thing. An **instrument you reach for at a moment** — **`/target-world-walk`**, a portfolio-completeness instrument: derive what a plan needs by replaying one concrete unit of work through the system you *intend to have*, instead of auditing the tickets you already wrote. It refuses a goal that points at a tracker item, classifies every need as covered / partial / missing / oral, and runs an ordering pass before anything is filed. And a **standing rule that is never invoked at all** — the flagellant rule, which reports a mistake as fact plus fix rather than apology, stops when the stopping condition is met instead of re-checking finished work, and spends care where an error cannot be taken back. It arrives through a `SessionStart` hook, so it composes with whatever output style you have chosen rather than replacing it. | [README](plugins/agent-handbook/README.md) |

...
|---|---|---|---|
| **agent-handbook** | 0.4.0 | A handbook you hand to your agent — teaching material for the general judgement calls: architecture decisions, planning, the rules that hold whatever is being built. Not tied to a language, a stack or a build system; nothing installs a runtime, opens a network connection or needs hardware. It holds two kinds of thing. An **instrument you reach for at a moment** — **`/target-world-walk`**, a portfolio-completeness instrument: derive what a plan needs by replaying one concrete unit of work through the system you *intend to have*, instead of auditing the tickets you already wrote. It refuses a goal that points at a tracker item, classifies every need as covered / partial / missing / oral, and runs an ordering pass before anything is filed. And two **standing rules that are never invoked at all** — the flagellant rule, which reports a mistake as fact plus fix rather than apology, stops when the stopping condition is met instead of re-checking finished work, spends care where an error cannot be taken back, and reports completion only by first naming what would still be outstanding if it were wrong; and the non-stop rule, which says do the work and report it rather than asking permission, write in the past tense, never hand back the turn while work remains, ask a question only where the work becomes pointless or unsafe without the answer, probe your own rights with a command before saying a task needs a human, report done only with proof, and never write farewell summaries. Both rules arrive through a `SessionStart` hook, so they compose with whatever output style you have chosen rather than replacing it. | [README](plugins/agent-handbook/README.md) |
| **agent-statusline** | 0.1.0 | One line at the bottom of Claude Code: how much of the context window is gone, how much of the weekly budget is spent, and which model is answering. Every figure is read from the payload the harness pipes to the script on stdin, so nothing is host-specific. Needs `node` and nothing else. | [README](plugins/agent-statusline/README.md) |
